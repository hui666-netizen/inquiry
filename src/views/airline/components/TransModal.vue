...
import { TransportMaintainFreight } from '@/api/cams-inquiry/Transport'
...
/** row: 当前行的数据 */
const row = ref<transTableRule>()

/** 用于区分操作类型（新增或编辑） */
const types = ref<'add' | 'edit'>()

/** 表单数据模型 */
const tableForm = ref<transTableRule>({
  AirlineCompanyCode: undefined,
  StartPlaceCode: undefined,
  StartTransit1Code: undefined,
  TransitStartPlaceCode: undefined,
  ...
  TransitMethod1Id: undefined,
  Transit2Code: undefined,
  TransitMethod2Id: undefined,
  Transit3Code: undefined,
  TransitMethod3Id: undefined,
...
const [register, { validate, setFieldsValue }] = useSjzyForm({
  ...
  schemas: [
        ...
        { required: true, message: '请输入直飞起运港', trigger: 'change' },
        {
          validator: (_rule: any, value: any, callback: (arg0?: Error | undefined) => void) => {
            if (tableForm.value.StartPlaceCode === tableForm.value.EndPlaceCode) {
              callback(new Error('直飞起运港和目的港口不能相同'))
            }
            else if (tableForm.value.StartPlaceCode === tableForm.value.TransitStartPlaceCode) {
    ...
    },
    {
      ...
      componentProps: portInterface,
      rules: [
        { required: true, message: '请输入转运起运港', trigger: 'change' },
        {
          validator: (_rule: any, value: any, callback: any) => {
            if (tableForm.value.TransitStartPlaceCode === tableForm.value.StartPlaceCode) {
              return callback(new Error('出发港口和转运起运港不能相同'))
            }
            callback()
          },
          trigger: 'change',
        ...
        { required: true, message: '请输入目的港口', trigger: 'change' },
        {
          validator: (_rule: any, value: any, callback: (arg0?: Error | undefined) => void) => {
            if (tableForm.value.EndPlaceCode === tableForm.value.StartPlaceCode) {
              callback(new Error('出发港口和目的港口不能相同'))
            }
            else if (tableForm.value.EndPlaceCode === tableForm.value.TransitStartPlaceCode) {
              callback(new Error('目的港口和转运起运港不能相同'))
            }
            else {
              callback()
            }
    ...
    },
    {
      ...
      componentProps: ({ formModel }) => {
        return {
          ...
          immediate: true,
          extra: { airCompanyId: formModel.airCompanyId },
        }
      },
      required: true,
      colProps: {
      ...
      rules: [
        {
          validator: (_rule: any, value: any, callback: (arg0?: Error | undefined) => void) => {
            ...
            else if (tableForm.value.Transit1Code === tableForm.value.StartPlaceCode) {
              callback(new Error('转运港1和出发港口不能相同'))
            }
            else if (tableForm.value.Transit1Code === tableForm.value.EndPlaceCode) {
              callback(new Error('转运港1和目的港口不能相同'))
            }
            else if (tableForm.value.Transit2Code === tableForm.value.Transit1Code) {
              callback(new Error('转运港1和转运港2不能相同'))
            }
            else if (tableForm.value.Transit3Code === tableForm.value.Transit1Code) {
              callback(new Error('转运港1和转运港3不能相同'))
            }
            else {
              callback()
            }
      ...
      rules: [
        {
          validator: (_rule: any, value: any, callback: (arg0?: Error | undefined) => void) => {
            ...
            else if (tableForm.value.Transit2Code === tableForm.value.StartPlaceCode) {
              callback(new Error('转运港2和出发港口不能相同'))
            }
            else if (tableForm.value.Transit2Code === tableForm.value.EndPlaceCode) {
              callback(new Error('转运港2和目的港口不能相同'))
            }
            else if (tableForm.value.Transit2Code === tableForm.value.Transit1Code) {
              callback(new Error('转运港1和转运港2不能相同'))
            }
            else if (tableForm.value.Transit2Code === tableForm.value.Transit3Code) {
              callback(new Error('转运港1和转运港3不能相同'))
            }
            else {
              callback()
    ...
    },
    {
      ...
      rules: [
        {
          validator: (_rule: any, value: any, callback: (arg0?: Error | undefined) => void) => {
            if (tableForm.value.Transit3Code === undefined || tableForm.value.Transit3Code === '') {
              callback()
            }
            else if (tableForm.value.Transit3Code === tableForm.value.StartPlaceCode) {
              callback(new Error('转运港3和出发港口不能相同'))
            }
            else if (tableForm.value.Transit3Code === tableForm.value.EndPlaceCode) {
              callback(new Error('转运港3和目的港口不能相同'))
            }
            else if (tableForm.value.Transit3Code === tableForm.value.Transit1Code) {
              callback(new Error('转运港3和转运港1不能相同'))
            }
            else if (tableForm.value.Transit3Code === tableForm.value.Transit2Code) {
              callback(new Error('转运港3和转运港2不能相同'))
            }
            else {
              callback()
            }
...
function onDrawerClick(
  type: 'add' | 'edit',
  searchData: transSearchRule | undefined,
  val?: transTableRule,
) {
  row.value = undefined
  setModalProps({
    title: types.value === 'add' ? '新增转运运价维护' : '修改转运运价维护',
    width: '1200px',
    height: 500,
    visible: true,
    cancelText: '取消',
  ...
  if (type === 'add') {
    ...
    tableForm.value.TransitStartPlaceCode = searchData?.TransitStartPlaceCode
    ...
    nextTick(() => {
      setFieldsValue({
        ...
        StartTransit1Code: val?.StartTransit1,
        TransitStartPlaceCode: val?.TransitStartPlace,
        CurrencyCode: val?.Currency,
        Transit1Code: val?.Transit1,
        Transit2Code: val?.Transit2,
        Transit3Code: val?.Transit3,
        DestinationTransitMethodId: val?.DestinationTransitMethod,
        TransitMethod1Id: val?.TransitMethod1,
        TransitMethod2Id: val?.TransitMethod2,
        TransitMethod3Id: val?.TransitMethod3,
        ...val,
      })
    })
  }
...
async function handleSave() {
  ...
  const payload = buildPayload(actionType, selectedRow, user, curTime)

  try {
    const res = await TransportMaintainFreight(payload)
...
function buildPayload(
  ...
  selectedRow: transTableRule | undefined,
  user: any,
  curTime: any,
) {
  return {
    ...
    TransitMethod1Id: tableForm.value.TransitMethod1Id ?? 0,
    TransitMethod2Id: tableForm.value.TransitMethod2Id ?? 0,
    TransitMethod3Id: tableForm.value.TransitMethod3Id ?? 0,
    IsInternal: tableForm.value.IsInternal ?? 0,
